...
@Component({
  selector: 'app-deo-meeting',
  templateUrl: './deo-meeting.component.html',
  styleUrls: ['./deo-meeting.component.scss']
})
export class DeoMeetingComponent implements OnDestroy, OnInit {
  ...
  ngOnInit(): void {
    ...
    this.userservice.getAllUsers().subscribe((res: any) => {
      ...
      let people  = filtered
      this.atendents = people.map((i) => { i.fullName ='Fullname:' + i.firstname + ' ' + i.surname +'||'+ 'District Code & Role:' +i.districtCode + ' '+'&'+' ' +i.userType + '||' +'Emis Code:' +i.emisNumber  ; return i; });
      this.editatendents = people.map((i) => { i.fullName ='Fullname:' + i.firstname + ' ' + i.surname +'||'+ 'District Code & Role:' +i.districtCode + ' '+'&'+' ' +i.userType + '||' +'Emis Code:' +i.emisNumber  ; return i; });
    });
    ...
    if(this.userRole == "DEO"){
      this.meetingService.getAllMeetings().subscribe((res: any) => {
        let district = this.districtCode
        const filtered = res.filter(function(e) {
          return ( e['districtCode'] == district && e['meetingTypeId'] == 5);
        });
        this.data = filtered;
